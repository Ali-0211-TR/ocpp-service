...
## Phase 1 — ✅ Security & Core Commands (DONE)
...
- **Решение:**
  ...
  - Если нет — разрешить (per OCPP spec), но вернуть `Invalid` в `id_tag_info` и логировать как аномалию
  - Добавлен `get_transaction()` и `get_id_tag_parent()` в ChargePointService
  - Добавлен `get_parent_id_tag()` в IdTagRepository trait + SeaORM реализация
- **Файлы:** `handlers/ocpp_v16/handle_stop_transaction.rs`, `services/charge_point.rs`, `id_tag/repository.rs`
- **Приоритет:** 🟡 Средний
...
- **Решение:** WS Ping каждые 30с в send_task через `tokio::select!`. `AtomicBool` трекает получение Pong. Нет Pong до следующего Ping → соединение закрывается. Метрика `ws_connections_dead{reason="pong_timeout"}`.
...
- **Решение:** `DefaultBodyLimit::max(1_048_576)` (1 MB) в `router.rs` — одна строка в middleware stack.
...
| Место | Описание |
|-------|----------|
| `handle_stop_transaction.rs` (v1.6) | ✅ IdTag авторизация реализована (TODO #10) |
...
- [x] 88 unit-тестов
- [x] WS Ping/Pong keepalive (30s interval + dead connection detection)
- [x] HTTP Body Size Limit (1 MB via DefaultBodyLimit)
- [x] IdTag authorization at StopTransaction (parent tag support)
